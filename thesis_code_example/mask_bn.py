...
if DATASET == 'imagenette':
    ...
    checkpoint = torch.load(os.path.join(MODEL_DIR,args.model+'_nette.pth'), map_location='cpu')
    model.load_state_dict(checkpoint['model_state_dict']) 
    args.patch_size = args.patch_size if args.patch_size>0 else 32     
elif  DATASET == 'imagenet':
    model = torch.nn.DataParallel(model)
    checkpoint = torch.load(os.path.join(MODEL_DIR,args.model+'_net.pth'), map_location='cpu')
...
print("Image Number:",len(val_dataset))
