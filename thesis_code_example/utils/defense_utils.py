...
def ds(inpt,net,block_size, size_to_certify, num_classes, threshold=0.2):
	...
	predictions = torch.zeros(inpt.size(0), num_classes).type(torch.int).cpu()
	batch = inpt.permute(0,2,3,1) #color channel last
	for pos in range(batch.shape[2]):
		out_c1 = torch.zeros(batch.shape).cpu()
		out_c2 = torch.zeros(batch.shape).cpu()
		...
		predictions += (softmx >= threshold).type(torch.int).cpu()
	...
	cert = torch.tensor(((val - valsecond >2*num_affected_classifications) | ((val - valsecond ==2*num_affected_classifications)&(idx < idxsecond)))).cpu()
	return torch.tensor(idx).cpu(), cert
